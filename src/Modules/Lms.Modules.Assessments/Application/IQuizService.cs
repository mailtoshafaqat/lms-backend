...
{
    Task<QuizDto?> GetByTopicAsync(Guid topicId, Guid? userId, CancellationToken ct = default);
    Task<QuizDto?> GetAsync(Guid quizId, Guid? userId, CancellationToken ct = default);
